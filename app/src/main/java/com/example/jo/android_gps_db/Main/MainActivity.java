...
import com.example.jo.android_gps_db.Location.GPS_Service;
...
public class MainActivity extends AppCompatActivity {
    Toast toast;
    EditText latitude, longitude, name;
    Button addBtn, nameSearchBtn, deleteBtn, selectAll, location;
    NumberPicker radius;

    TextView settingRadius;
        ...
        public void onClick(View view) {
            switch (view.getId()) {
                case R.id.addBtn: //DB추가 버튼튼
                    ...
                    try {
                        String result = new CustomTask().execute(addLatitude, addLongitude, addRadius, addname, "add").get();
                ...
                case R.id.deleteBtn: // 삭제
                    ...
                    try {
                        String result = new CustomTask().execute(delLatitude, delLongitude, delRadius, delhname, "delete").get();
                        if (result.equals("delete")) {
                            toast.setText("삭제 되었습니다.");
                            toast.show();
